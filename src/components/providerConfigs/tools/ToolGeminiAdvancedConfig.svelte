<script>
  ...
  function saveKeys() {
    ...
    const plainAdditionalKeys = Array.from($state.snapshot(additionalApiKeys))

    updateSettings({
      geminiAdvancedApiKey: apiKey,
  ...
  onMount(() => {
    additionalApiKeys = settings.geminiAdvancedAdditionalApiKeys || []
  })

  function removeKey(index) {
    additionalApiKeys = additionalApiKeys.filter((_, i) => i !== index)
    saveKeys()
  }
...
<div class="flex flex-col gap-4">
  <div class="flex flex-col gap-1">
    ...
    <div class="flex items-end group">
      <div class="flex-1">
        <ApiKeyInputMulti
          ...
          id="tool-gemini-advanced-api-key-main"
    ...
    <!-- Additional API Keys -->
    {#each additionalApiKeys as key, index}
      <div class="flex items-end group">
        <div class="flex-1">
          <ApiKeyInputMulti
            bind:apiKey={additionalApiKeys[index]}
            label=""
            onSave={saveKeys}
            placeholder={$t(
            ...
            id={`tool-gemini-advanced-additional-key-${index}`}
            showStatus={false}
          />
        </div>
...
<div class="flex flex-col gap-2 relative z-50">
  <ProviderModelSelect
    ...
    inputId="gemini-tool-model"
    ariaLabel="Search Gemini model"
    onModelChange={handleModelSave}
  />
</div>
