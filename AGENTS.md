...
- **`aiSdkAdapter.js`** - Core adapter that maps provider IDs and settings to AI SDK models. Functions: `getAISDKModel()`, `generateContent()`, `generateContentStream()`. Supports 8+ providers (Gemini, OpenAI, Codex, Groq, OpenRouter, Ollama, DeepSeek, LM Studio).
